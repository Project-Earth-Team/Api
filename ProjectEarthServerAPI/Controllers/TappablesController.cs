...
namespace ProjectEarthServerAPI.Controllers
{
	...
	public class TappablesController : Controller
	{
		...
		public async Task<IActionResult> Post(int x, int y)
		{
			...
			{
				Log.Error("[Tappables] no json file for tappable type" + type +
						  " exists in data/tappables. Using backup of dirt (f0617d6a-c35a-5177-fcf2-95f67d79196d)");
			...
			{
				rewards[i] = new RewardComponent() { Amount = random.Next(1, 3), Id = new Guid(targetDropSet[i]) };
			}
			...
			var response = new TappableResponse()
			{
				result = new TappableResponse.Result()
				{
					token = new Token()
					{
						...
						lifetime = "Persistent",
						rewards = new Rewards() { ExperiencePoints = 400, Inventory = rewards }
					}
				}
			};
